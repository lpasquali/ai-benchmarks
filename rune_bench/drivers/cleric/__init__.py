...
"""Cleric driver client — autonomous SRE and infrastructure reliability orchestration."""
...
class ClericDriverClient:
    """Cleric autonomous SRE agent. Requires enterprise API access.

    Configure via environment variables:
        RUNE_CLERIC_API_KEY   — API key/token
    """
    ...
    def _check_auth(self) -> None:
        """Raise RuntimeError if RUNE_CLERIC_API_KEY is not set."""
        api_key = os.getenv("RUNE_CLERIC_API_KEY")
        if not api_key:
            raise RuntimeError(
                "Cleric requires an enterprise contract or API access. "
                f"Visit {self.ONBOARDING_URL} to get started. "
                "Once provisioned, set RUNE_CLERIC_API_KEY."
            )
