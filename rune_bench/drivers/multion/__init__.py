...
"""MultiOn driver client — autonomous web navigation and action orchestration."""
...
class MultiOnDriverClient:
    """MultiOn autonomous web agent. Requires enterprise API access.

    Configure via environment variables:
        RUNE_MULTION_API_KEY   — API key/token
    """

    ONBOARDING_URL = "https://www.multion.ai/"

    def __init__(self, *, transport: DriverTransport | None = None, **kwargs) -> None:
        self._transport: DriverTransport = transport or make_driver_transport("multion")
    ...
    def _check_auth(self) -> None:
        """Raise RuntimeError if RUNE_MULTION_API_KEY is not set."""
        api_key = os.getenv("RUNE_MULTION_API_KEY")
        if not api_key:
            raise RuntimeError(
                "MultiOn requires an enterprise contract or API access. "
                f"Visit {self.ONBOARDING_URL} to get started. "
                "Once provisioned, set RUNE_MULTION_API_KEY."
            )
