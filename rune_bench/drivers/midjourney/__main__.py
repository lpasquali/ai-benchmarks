...
"""Actual implementation for midjourney driver."""
...
from .runner import MidjourneyRunner
...
def _handle_ask(params: dict) -> dict:
    ...
    if not api_key:
        ...
        raise RuntimeError("RUNE_MIDJOURNEY_API_KEY not set")
    
    api_base = os.getenv("RUNE_MIDJOURNEY_API_BASE")
    ...
    try:
        runner = MidjourneyRunner(api_key=api_key)
    except TypeError:
        # Some might take base_url instead or as well
        runner = MidjourneyRunner(api_key=api_key, api_base=api_base)
    
    answer = runner.ask(question, model=model, backend_url=backend_url, backend_type=backend_type)
