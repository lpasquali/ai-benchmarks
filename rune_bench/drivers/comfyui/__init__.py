...
"""ComfyUI driver client — granular nodes-based visual AI orchestration."""
...
class ComfyUIDriverClient:
    """ComfyUI visual AI platform. Requires enterprise API access.

    Configure via environment variables:
        RUNE_COMFYUI_API_KEY   — API key/token
    """
    ...
    def ask_structured(
        ...
        return AgentResult(
            answer=answer,
            result_type=result.get("result_type", "image"),
    ...
    def _check_auth(self) -> None:
        """Raise RuntimeError if RUNE_COMFYUI_API_KEY is not set."""
        api_key = os.getenv("RUNE_COMFYUI_API_KEY")
        if not api_key:
            raise RuntimeError(
                "ComfyUI requires an enterprise contract or API access. "
                f"Visit {self.ONBOARDING_URL} to get started. "
                "Once provisioned, set RUNE_COMFYUI_API_KEY."
            )
