...
async def test_backend_type_checks():
    ...
    with pytest.raises(
        RuntimeError, match="invalid request type for agentic-agent backend"
    ):
        await _run_agentic_backend(MagicMock())

    # Test _run_benchmark_backend type check
    with pytest.raises(
        RuntimeError, match="invalid request type for benchmark backend"
    ):
    ...
    with pytest.raises(
        RuntimeError, match="invalid request type for cost-estimate backend"
    ):
...
async def test_api_application_execute_kinds():
    store = MagicMock()
    app = RuneApiApplication(
        store=store, security=ApiSecurityConfig(auth_disabled=True, tenant_tokens={})
    )
    ...
    store.update_job.assert_any_call(
        "j1", status="succeeded", result_payload={"ok": True}
    )
    ...
    store.update_job.assert_any_call(
        "j5", status="failed", error="unsupported job kind: unknown"
    )
...
def test_auth_failure_logging():
    ...
    app = RuneApiApplication(
        store=MagicMock(),
        security=ApiSecurityConfig(auth_disabled=False, tenant_tokens={"t1": "hash1"}),
    )
    handler_class = app.create_handler()
    handler = handler_class.__new__(handler_class)
...
async def test_verify_attestation_failure():
    ...
    mock_driver.verify.return_value = AttestationResult(
        passed=False, pcr_digest=None, message="failed"
    )

    with patch("rune_bench.attestation.factory.get_driver", return_value=mock_driver):
        with pytest.raises(
            RuntimeError, match="Attestation failed for scheduling target 't1': failed"
        ):
...
def test_api_handler_post_jobs_missing_tenant():
    ...
    app = RuneApiApplication(
        store=MagicMock(),
        security=ApiSecurityConfig(auth_disabled=False, tenant_tokens={}),
    )
    handler_class = app.create_handler()
    handler = handler_class.__new__(handler_class)
    handler.path = "/v1/jobs"
...
def test_api_handler_post_jobs_rate_limited():
    ...
    with patch.object(
        app, "_enforce_request_rate_limit", side_effect=RequestRateLimited()
    ):
        handler_class = app.create_handler()
        handler = handler_class.__new__(handler_class)
