...
async def test_run_preflight_cost_check():
    ...
    res = await run_preflight_cost_check(vastai=True, max_dph=1, min_dph=0, backend_mode="http", http_client=mock_client)
    assert res == {"cost": 2.0}
    
    # http mode missing client
    with pytest.raises(RuntimeError, match="http_client is required"):
        await run_preflight_cost_check(vastai=True, max_dph=1, min_dph=0, backend_mode="http")
...
def test_provision_vastai_backend():
    ...
    with patch("rune_bench.workflows.InstanceManager") as mock_mgr_cls, \
        ...
        mock_sel.select.return_value = MagicMock(name="m1", vram_mb=5000, required_disk_gb=10)
        ...
        mock_mgr.find_reusable_running_instance.return_value = {"id": "123", "gpu_total_ram": 10000}
        with patch("rune_bench.workflows.OllamaBackend.extract_service_url", return_value="http://u"):
            res = provision_vastai_backend(
                mock_sdk, template_hash="t1", min_dph=0, max_dph=1, reliability=0.9, confirm_create=lambda: True
            )
            assert res.contract_id == "123"
            mock_warmup.assert_called_once()

        # Case 2: Reusable instance triggered fallback (empty ID)
        mock_mgr.find_reusable_running_instance.return_value = {"id": "", "gpu_total_ram": 10000}
        with patch("rune_bench.workflows.OllamaBackend.extract_service_url", return_value="http://u"):
            res = provision_vastai_backend(
                mock_sdk, template_hash="t1", min_dph=0, max_dph=1, reliability=0.9, confirm_create=lambda: True
            )
        ...
        mock_mgr.find_reusable_running_instance.return_value = {"id": "123", "gpu_total_ram": 10000}
        mock_sel.select.side_effect = [RuntimeError("select fail"), MagicMock(name="m1", vram_mb=5000, required_disk_gb=10)]
        with patch("rune_bench.workflows.OllamaBackend.extract_service_url", return_value="http://u"):
            res = provision_vastai_backend(
                mock_sdk, template_hash="t1", min_dph=0, max_dph=1, reliability=0.9, confirm_create=lambda: True
            )
        ...
        mock_sel.select.return_value = MagicMock(name="m1", vram_mb=5000, required_disk_gb=10)
        with patch("rune_bench.workflows.list_backend_models", return_value=[]), \
             patch("rune_bench.workflows.OllamaBackend.extract_service_url", return_value="http://u"):
        ...
        with patch("rune_bench.workflows.OllamaBackend.extract_service_url", return_value=None):
            res = provision_vastai_backend(
                mock_sdk, template_hash="t1", min_dph=0, max_dph=1, reliability=0.9, confirm_create=lambda: True
            )
...
def test_extract_ollama_service_url():
    with patch("rune_bench.workflows.OllamaBackend.extract_service_url", return_value="url"):
...
def test_provision_vastai_backend_user_abort():
    ...
    with patch("rune_bench.workflows.InstanceManager") as mock_mgr_cls, \
        ...
        mock_mgr = mock_mgr_cls.return_value
        mock_mgr.find_reusable_running_instance.return_value = None # Force new creation
