...
def test_cli_run_http_error_ollama():
    ...
    with patch("rune._run_http_job_with_progress", return_value=payload):
        result = runner.invoke(
            app, ["--backend", "http", "run-llm-instance", "--backend-url", "http://u"]
        )
        assert result.exit_code == 1
...
def test_apply_model_limits_with_env_mock():
    ...
    _apply_model_limits(
        ModelCapabilities(model_name="m1", context_window=200, max_output_tokens=100)
    )
...
def test_cli_run_benchmark_agent_error():
    mock_runner = MagicMock()
    mock_runner.ask_structured = AsyncMock(
...
def test_cli_run_benchmark_with_artifacts():
    payload = {
        "status": "success",
        "result": {"answer": "ok", "result_type": "text", "artifacts": ["art1"]},
    }
    with patch("rune._run_http_job_with_progress", return_value=payload):
        result = runner.invoke(
            app,
