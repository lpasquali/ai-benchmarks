...
def test_get_cost_estimate_cloud(monkeypatch):
    ...
    req = CostEstimationRequest(
        estimated_duration_seconds=3600, aws=True, model="g4dn.xlarge"
    )
    ...
    req = CostEstimationRequest(
        estimated_duration_seconds=3600, gcp=True, model="n1-standard-4"
    )
    res = get_cost_estimate(req)
    ...
    req = CostEstimationRequest(
        estimated_duration_seconds=3600, azure=True, model="Standard_NC6s_v3"
    )
    res = get_cost_estimate(req)
