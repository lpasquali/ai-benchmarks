...
async def test_run_benchmark_telemetry_no_cost():
    ...
    req = RunBenchmarkRequest(
        model="m", question="q", backend_url="u", 
    ...
    mock_result.telemetry = RunTelemetry(tokens=MagicMock(), latency=[], cost_estimate_usd=None)
    ...
    with patch("rune_bench.api_backend._make_resource_provider_for_benchmark", return_value=mock_provider):
        with patch("rune_bench.api_backend._make_agent_runner", return_value=mock_runner):
...
def test_make_resource_provider_no_backend_type():
    req = RunBenchmarkRequest(
        model="m", question="q", backend_url="http://u", 
        provisioning=None, backend_warmup=True, 
        backend_warmup_timeout=10, kubeconfig="k"
    )
