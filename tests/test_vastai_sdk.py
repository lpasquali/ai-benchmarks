...
def test_search_offers(vastai, respx_mock):
    respx_mock.get(
        "https://console.vast.ai/api/v0/bundles/?q=%7B%22foo%22%3A%20%22bar%22%7D"
    ).respond(json={"offers": [{"id": 123}]})
...
def test_create_instance(vastai, respx_mock):
    ...
    res = vastai.create_instance(
        id="456", disk=10.0, env="-e FOO=bar", image="ubuntu:latest"
    )
