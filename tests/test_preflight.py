...
async def test_preflight_local_backend_returns_estimate():
    ...
    with patch(
        "rune_bench.common.costs.CostEstimator.estimate", new_callable=AsyncMock
    ) as mock_est:
...
async def test_preflight_re_raises_fail_closed_error():
    with patch(
        "rune_bench.common.costs.CostEstimator.estimate",
        side_effect=FailClosedError("closed"),
    ):
...
async def test_cli_preflight_spend_gate_aborted(monkeypatch):
    ...
    with patch("rune.run_preflight_cost_check", side_effect=mock_preflight):
        ...
        monkeypatch.setattr(
            "rune.evaluate_spend_gate", lambda *a, **k: SpendGateAction.BLOCK
        )
        with pytest.raises(rune_cli.typer.Exit):
            await rune_cli._run_preflight_cost_check(
                vastai=True, max_dph=3.0, min_dph=2.0, yes=False
            )
...
async def test_cli_preflight_spend_gate_accepted(monkeypatch):
    ...
    with patch("rune.run_preflight_cost_check", side_effect=mock_preflight):
        ...
        await rune_cli._run_preflight_cost_check(
            vastai=True, max_dph=3.0, min_dph=2.0, yes=True
        )


@pytest.mark.asyncio
async def test_cli_preflight_unavailable_warning(monkeypatch):
    async def mock_err(*a, **k):
...
async def test_cli_preflight_fail_closed_error(monkeypatch):
    async def mock_fail(*a, **k):
        raise FailClosedError("No driver")
...
async def test_cli_preflight_not_vastai_noop():
    ...
    with patch(
        "rune.run_preflight_cost_check", side_effect=AsyncMock(return_value={})
    ) as mock_run:
        await rune_cli._run_preflight_cost_check(
            vastai=False, max_dph=3.0, min_dph=2.0, yes=True
        )
