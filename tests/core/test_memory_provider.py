...
def test_semantic_memory():
    provider = MemoryProvider()
    provider.store_semantic(
        "CrashLoopBackOff",
        "Usually indicates a missing env var or failing startup script.",
    )

    assert (
        provider.retrieve_semantic("CrashLoopBackOff")
        == "Usually indicates a missing env var or failing startup script."
    )
...
def test_procedural_memory():
    provider = MemoryProvider()
    provider.cache_procedure(
        "rollback_deployment",
        ["kubectl rollout undo deploy/web", "kubectl get pods -w"],
    )
