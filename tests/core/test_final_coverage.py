...
def test_api_contracts_to_dict():
    ...
    assert (
        RunStatusResponse(
            job_id="j", status="s", message="m", created_at=1.0
        ).to_dict()["job_id"]
        == "j"
    )
    ...
    assert (
        RunLLMInstanceRequest(provisioning=None, backend_url=None).to_dict()[
            "provisioning"
        ]
        is None
    )
...
def test_api_server_from_env_postgres():
    with patch.dict(
        os.environ,
        {
            "RUNE_DATABASE_URL": "postgresql://user:pass@host/db",
            "RUNE_API_AUTH_DISABLED": "1",
        },
    ):
...
def test_api_server_from_env_invalid_db():
    with patch.dict(
        os.environ,
        {
            "RUNE_DATABASE_URL": "mysql://user:pass@host/db",
            "RUNE_API_AUTH_DISABLED": "1",
        },
    ):
...
def test_holmes_fetch_model_limits_error_coverage():
    with patch("pathlib.Path.exists", return_value=True):
        driver = HolmesDriverClient(kubeconfig=Path("/tmp/k"))
        with patch(
            "rune_bench.drivers.holmes.get_backend", side_effect=Exception("fail")
        ):
            res = driver._fetch_model_limits(model="m", backend_url="http://u")
            assert res == {}
...
def test_cli_run_agent_empty_answer():
    ...
    with patch("rune._run_http_job_with_progress", return_value=payload):
        result = runner.invoke(
            app, ["--backend", "http", "run-agentic-agent", "--question", "q"]
        )
...
def test_cli_run_agent_artifacts_print_coverage():
    ...
    payload = {"status": "success", "result": {"answer": "ok", "artifacts": ["art1"]}}
    from rune import app
    from typer.testing import CliRunner
...
def test_config_attestation_merge():
    ...
    with patch("rune_bench.common.config.get_raw_config") as mock_raw:
        ...
        mock_raw.return_value = {
            "attestation": {"pcr_policy_path": "p1"},
            "defaults": {},
        }
...
def test_benchmark_teardown_error_coverage():
    ...
    with patch("rune.provision_vastai_backend", return_value=mock_res):
        with patch(
            "rune._fetch_model_capabilities",
            return_value=ModelCapabilities(model_name="m", context_window=100),
        ):
            with patch(
                "rune.stop_vastai_instance", side_effect=RuntimeError("stop fail")
            ):
...
def test_k8sgpt_json_error():
    ...
    with (
        ...
        patch("shutil.which", return_value="/bin/k8sgpt"),
    ):
