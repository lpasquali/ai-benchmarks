...
def test_reflection_agent_ask():
    ...
    answer = runner.ask("What is 2+2?", model="dummy", backend_url=None)
...
def test_reflection_agent_ask_multiple_reflections():
    ...
    assert (
        answer
        == "Reflected and improved: Reflected and improved: Draft response to: What is 2+2?"
    )


def test_reflection_agent_ask_structured():
    runner = ReflectionAgentRunner(max_reflections=1)
    result = runner.ask_structured("Test query", model="dummy", backend_url=None)
