...
def test_tpm2_quote_timeout():
    ...
    with patch("shutil.which", return_value="/bin/tpm2_quote"):
        driver = TPM2Driver()
        with patch(
            "subprocess.run", side_effect=TimeoutExpired(cmd=["tpm2_quote"], timeout=5)
        ):
    ...
    with patch("shutil.which", return_value="/bin/tpm2_quote"):
        ...
        with patch(
            ...
            return_value=CompletedProcess(
                ["tpm2_quote"], 1, stdout="", stderr="failed"
            ),
        ):
            res = driver.verify("t1")
            assert not res.passed
...
def test_tpm2_checkquote_disappeared(tmp_path):
    ...
    with patch("shutil.which", return_value="/bin/tpm2_tools"):
        ...
        with patch("subprocess.run", side_effect=mock_run):
            with pytest.raises(
                RuntimeError, match="tpm2_checkquote binary disappeared"
            ):
                driver.verify("t1")
...
def test_tpm2_verify_success_no_policy():
    ...
    with patch("shutil.which", return_value="/bin/tpm2_quote"):
        ...
        with patch(
            "subprocess.run",
            return_value=CompletedProcess(["tpm2_quote"], 0, stdout="abc", stderr=""),
        ):
            res = driver.verify("t1")
            assert res.passed
