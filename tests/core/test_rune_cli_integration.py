...
async def test_run_llm_instance_paths(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ExistingOllamaServer(url="http://x", model_name="m"),
    )
...
async def test_run_agentic_agent_paths(monkeypatch, tmp_path):
    ...
    fake_client = type(
        ...
        (),
        {
            "submit_agentic_agent_job": lambda self, payload, idempotency_key=None: (
                "job-1"
            ),
            "wait_for_job": mock_wait,
        },
    ...
    monkeypatch.setattr(
        rune,
        "use_existing_backend_server",
        lambda *_a, **_k: ExistingOllamaServer(url="http://x", model_name="m"),
    )
