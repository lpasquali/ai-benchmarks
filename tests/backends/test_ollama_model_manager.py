...
async def test_api_backend_functions(tmp_path, monkeypatch):
    ...
    assert await api_backend.run_llm_instance(req) == {
        "mode": "existing",
        "backend_url": "norm:u",
    }

    mock_prov_ollama = AsyncMock()
    mock_prov_ollama.provision.return_value = ProvisioningResult(
        backend_url="http://x", model="m", provider_handle=3
    )
    monkeypatch.setattr(
        api_backend,
    ...
    monkeypatch.setattr(
        api_backend,
        "warmup_backend_model",
        lambda *_args, **_kwargs: warmed.append(True) or "m",
    )

    mock_agent_run = AsyncMock()
    mock_agent_run.ask_structured.return_value = AgentResult(answer="answer")
    monkeypatch.setattr(
        api_backend, "get_agent", lambda *_args, **_kwargs: mock_agent_run
    )

    areq = api_backend.RunAgenticAgentRequest(
        question="q",
        model="m",
        backend_url="http://x",
        backend_warmup=True,
    ...
    mock_prov_bench.provision.return_value = ProvisioningResult(
        backend_url="http://existing", model="m"
    )
    monkeypatch.setattr(
        api_backend,
    ...
    breq = api_backend.RunBenchmarkRequest(
        provisioning=None,
        backend_url="u",
        question="q",
        model="m",
    ...
    mock_prov_fail.provision.return_value = ProvisioningResult(
        backend_url=None, model="m", provider_handle=7
    )
    monkeypatch.setattr(
        api_backend,
        "_make_resource_provider_for_benchmark",
