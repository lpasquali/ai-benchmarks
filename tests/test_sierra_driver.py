...
"""Tests for rune_bench.drivers.sierra — driver client and subprocess entry point."""
...
import rune_bench.drivers.sierra.__main__ as sierra_main
...
def test_handle_ask_raises_without_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("RUNE_SIERRA_API_KEY", raising=False)
    with pytest.raises(RuntimeError, match="RUNE_SIERRA_API_KEY"):
        sierra_main._handle_ask({"question": "test", "model": "m"})


def test_handle_ask_raises_not_implemented_with_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("RUNE_SIERRA_API_KEY", "test-key")
...
def test_handle_info_returns_driver_metadata() -> None:
    result = sierra_main._handle_info({})
    assert result["name"] == "sierra"
...
def test_main_processes_info_request(
    ...
    assert response["result"]["name"] == "sierra"
...
def test_main_handles_invalid_json(
    ...
    monkeypatch.setattr(sierra_main.sys, "stdin", io.StringIO("not-json\n"))
...
def test_main_skips_empty_lines(
    ...
    monkeypatch.setattr(sierra_main.sys, "stdin", io.StringIO("\n\n   \n"))
...
def test_main_ask_error_propagated(
    ...
    response = json.loads(capsys.readouterr().out.strip())
    assert response["status"] == "error"
    assert "RUNE_SIERRA_API_KEY" in response["error"]


# ---------------------------------------------------------------------------
# SierraDriverClient — with mocked transport
# ---------------------------------------------------------------------------
...
def test_driver_client_raises_without_api_key() -> None:
    ...
    os.environ.pop("RUNE_SIERRA_API_KEY", None)
