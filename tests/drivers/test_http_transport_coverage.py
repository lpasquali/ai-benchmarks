...
async def test_async_http_transport_happy_path():
    transport = AsyncHttpTransport(
        "http://localhost:8080", api_token="t1", tenant="tenant1"
    )
    ...
    with patch(
        "rune_bench.drivers.http.make_async_http_request", AsyncMock()
    ) as mock_req:
...
async def test_async_http_transport_no_job_id():
    ...
    with patch(
        "rune_bench.drivers.http.make_async_http_request", AsyncMock(return_value={})
    ):
