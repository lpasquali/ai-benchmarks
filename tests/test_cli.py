...
import rune
from rune import app, _is_containerized, _find_free_port, _resolve_serve_port, _enable_debug_if_requested
...
def test_cli_db_migrate_postgres():
    with patch("rune_bench.storage.migrate_to_postgres.migrate_to_postgres") as mock_migrate:
        ...
        result = runner.invoke(app, ["db", "migrate-to-postgres", "--target", "postgresql://user:pass@host/db"])
        assert result.exit_code == 0
...
def test_cli_db_migrate_postgres_dry_run():
    with patch("rune_bench.storage.migrate_to_postgres.migrate_to_postgres") as mock_migrate:
        ...
        result = runner.invoke(app, ["db", "migrate-to-postgres", "--target", "postgresql://user:pass@host/db", "--dry-run"])
        assert result.exit_code == 0
...
def test_cli_run_agentic_agent_local(mock_get_agent):
    ...
    with patch("rune._warmup_ollama_model"), \
        ...
        result = runner.invoke(app, ["run-agentic-agent", "--model", "m1", "--question", "q1"])
        assert result.exit_code == 0
...
def test_cli_ollama_list_models_http(mock_client_cls):
    ...
    result = runner.invoke(app, ["--backend", "http", "ollama-list-models", "--backend-url", "http://remote"])
    assert result.exit_code == 0
...
def test_cli_ollama_list_models_local():
    with patch("rune.list_backend_models", return_value=["m1"]), \
         patch("rune.list_running_backend_models", return_value=["m1"]):
        result = runner.invoke(app, ["ollama-list-models", "--backend-url", "http://local"])
        assert result.exit_code == 0
...
def test_apply_model_limits():
    ...
    caps = ModelCapabilities(model_name="m1", context_window=1000, max_output_tokens=500)
...
def test_cli_run_benchmark_local():
    with patch("rune.use_existing_backend_server") as mock_use, \
        ...
        result = runner.invoke(app, ["run-benchmark", "--model", "m1", "--question", "q1"])
        assert result.exit_code == 0
...
def test_cli_run_benchmark_http(mock_http_client, mock_run_progress):
    mock_run_progress.return_value = {"result": {"answer": "HTTP answer", "result_type": "text", "artifacts": []}}
    result = runner.invoke(app, ["--backend", "http", "run-benchmark", "--model", "m1", "--question", "q1"])
    assert result.exit_code == 0
...
def test_cli_run_agentic_agent_http(mock_http_client, mock_run_progress):
    mock_run_progress.return_value = {"result": {"answer": "HTTP agent answer", "result_type": "text", "artifacts": []}}
    result = runner.invoke(app, ["--backend", "http", "run-agentic-agent", "--model", "m1", "--question", "q1"])
    assert result.exit_code == 0
...
def test_cli_run_llm_instance_http_vastai(mock_http_client, mock_run_progress):
    mock_run_progress.return_value = {"result": {"mode": "vastai", "contract_id": "c1", "backend_url": "http://vast", "model_name": "m1"}}
...
def test_cli_run_llm_instance_http_existing(mock_http_client, mock_run_progress):
    mock_run_progress.return_value = {"result": {"mode": "existing", "backend_url": "http://existing"}}
    result = runner.invoke(app, ["--backend", "http", "run-llm-instance", "--backend-url", "http://existing"])
    assert result.exit_code == 0
...
def test_print_vastai_result_with_caps():
    ...
    result = VastAIProvisioningResult(
        offer_id=1, total_vram_mb=8000, model_name="m1", model_vram_mb=4000,
        required_disk_gb=40, template_env="env", contract_id="c1",
        details=ConnectionDetails(contract_id="c1", status="running", ssh_host="h", ssh_port=22, machine_id="m1"),
...
def test_cli_run_llm_instance_vastai(mock_run):
    ...
    mock_run.return_value = VastAIProvisioningResult(
        ...
        template_env="env",
        contract_id="c1",
        details=ConnectionDetails(contract_id="c1", status="running", ssh_host="localhost", ssh_port=2222, machine_id="m1"),
        backend_url="http://vast",
    )
...
def test_cli_run_llm_instance_existing():
    with patch("rune.use_existing_backend_server") as mock_use:
        ...
        result = runner.invoke(app, ["run-llm-instance", "--backend-url", "http://existing"])
        assert result.exit_code == 0
        assert "Existing server" in result.stdout
...
@patch("rune._warmup_ollama_model")
def test_cli_run_benchmark_vastai_success(mock_warmup, mock_stop, mock_get_agent, mock_run_vast):
    ...
    mock_stop.return_value = TeardownResult(contract_id="c1", destroyed_instance=True, verification_ok=True)
...
def test_run_preflight_cost_check_no_vastai():
    from rune import _run_preflight_cost_check
    import asyncio
    asyncio.run(_run_preflight_cost_check(vastai=False, max_dph=1.0, min_dph=0.5, yes=False))
...
def test_run_preflight_cost_check_invalid_threshold():
    ...
    with patch.dict(os.environ, {"RUNE_SPEND_WARNING_THRESHOLD": "invalid"}):
        import asyncio
        asyncio.run(_run_preflight_cost_check(vastai=True, max_dph=1.0, min_dph=0.5, yes=True))
