...
from rune_bench.api_contracts import RunBenchmarkRequest, Provisioning, VastAIProvisioning, RunLLMInstanceRequest, RunAgenticAgentRequest
...
async def test_run_preflight_cost_check_failures(monkeypatch):
    ...
    with patch("rune.run_preflight_cost_check", side_effect=RuntimeError("fail")):
        with patch("rune.console"):
            ...
            await rune._run_preflight_cost_check(vastai=True, max_dph=1, min_dph=0, yes=True)
...
async def test_run_preflight_cost_check_interactive(monkeypatch):
    res = {"projected_cost_usd": 10.0, "cost_driver": "test", "resource_impact": "high", "warning": "watch out"}
    with patch("rune.run_preflight_cost_check", AsyncMock(return_value=res)):
        with patch("rune.console") as mock_console:
            with patch("rune.evaluate_spend_gate", return_value=rune.SpendGateAction.PROMPT):
...
def test_make_resource_provider_vastai():
    v = VastAIProvisioning(template_hash="t", min_dph=1, max_dph=2, reliability=0.9, stop_instance=True)
    req = RunBenchmarkRequest(provisioning=Provisioning(vastai=v), backend_url=None, question="q", model="m", backend_warmup=False, backend_warmup_timeout=0, kubeconfig="")
...
async def test_run_benchmark_http_mode():
    with patch("rune.BACKEND_MODE", "http"):
        ...
        mock_client.submit_benchmark_job.return_value = {"result": {"answer": "Some answer", "result_type": "text"}}
        with patch("rune._http_client", return_value=mock_client):
            with patch("rune.console"):
