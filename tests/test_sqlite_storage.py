...
def test_sqlite_adapter_create_job(adapter):
    job_id, created = adapter.create_job(tenant_id="t1", kind="benchmark", request_payload={"q": "a"})
    assert job_id is not None
    assert created is True
    
    # Idempotent (not found)
    job_id2, created2 = adapter.create_job(tenant_id="t1", kind="benchmark", request_payload={"q": "a"}, idempotency_key="k1")
    assert created2 is True
    
    # Idempotent (found)
    job_id3, created3 = adapter.create_job(tenant_id="t1", kind="benchmark", request_payload={"q": "a"}, idempotency_key="k1")
    assert job_id3 == job_id2
    assert created3 is False

def test_sqlite_adapter_get_job(adapter):
    job_id, _ = adapter.create_job(tenant_id="t1", kind="benchmark", request_payload={"q": "a"})
...
def test_sqlite_adapter_update_job(adapter):
    ...
    adapter.update_job(job_id, status="success", result_payload={"ans": "a"}, message="done", error="err")
    job = adapter.get_job(job_id)
...
def test_sqlite_adapter_audit_artifacts(adapter):
    ...
    # record
    art_id = adapter.record_audit_artifact(job_id=job_id, kind="sbom", name="n1", content=b"data")
    assert art_id is not None
    
    # invalid kind
    with pytest.raises(ValueError, match="unknown audit artifact kind"):
        adapter.record_audit_artifact(job_id=job_id, kind="invalid", name="n1", content=b"d")
...
def test_sqlite_adapter_chain_state(adapter):
    ...
    adapter.record_chain_node_transition(job_id=job_id, node_id="n2", status="success", started_at=1.0, finished_at=2.0, error="err")
    state = adapter.get_chain_state(job_id)
    ...
    with pytest.raises(RuntimeError, match="not initialized"):
        adapter.record_chain_node_transition(job_id="missing", node_id="n1", status="success")
        
    with pytest.raises(RuntimeError, match="not found"):
        adapter.record_chain_node_transition(job_id=job_id, node_id="missing", status="success")
...
def test_compute_overall_chain_status():
    ...
    assert SS._compute_overall_chain_status([{"status": "success"}, {"status": "skipped"}]) == "success"
