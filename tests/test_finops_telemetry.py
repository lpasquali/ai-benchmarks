...
def rune_api_server():
    ...
    def run_agentic(request):
        ...
        return {"answer": f"ok:{request.question}", "telemetry": {"tokens": {"total": 100}}}

    app = RuneApiApplication(
        store=store,
        security=ApiSecurityConfig(auth_disabled=False, tenant_tokens={"tenant-a": "token-a"}),
        backend_functions={
...
async def test_api_finops_simulate(rune_api_server):
    ...
    store.update_job(job_id, status="succeeded", result_payload={"eval_count": 100, "prompt_eval_count": 50})
