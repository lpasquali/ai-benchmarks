...
"""Tests for rune_bench.drivers.harvey — driver client and subprocess entry point."""
...
import rune_bench.drivers.harvey.__main__ as harv_main
...
def test_handle_ask_raises_without_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("RUNE_HARVEY_API_KEY", raising=False)
    with pytest.raises(RuntimeError, match="RUNE_HARVEY_API_KEY"):
...
def test_main_processes_info_request(
    ...
    assert response["result"]["name"] == "harvey"
...
def test_main_handles_invalid_json(
    ...
    monkeypatch.setattr(harv_main.sys, "stdin", io.StringIO("not-json\n"))
...
def test_main_skips_empty_lines(
    ...
    monkeypatch.setattr(harv_main.sys, "stdin", io.StringIO("\n\n   \n"))
...
def test_main_ask_error_propagated(
    ...
    response = json.loads(capsys.readouterr().out.strip())
    assert response["status"] == "error"
    assert "RUNE_HARVEY_API_KEY" in response["error"]


# ---------------------------------------------------------------------------
# HarveyDriverClient — with mocked transport
# ---------------------------------------------------------------------------
...
def test_driver_client_raises_without_api_key() -> None:
    ...
    os.environ.pop("RUNE_HARVEY_API_KEY", None)
